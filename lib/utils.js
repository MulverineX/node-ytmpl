...
exports.parseBody = (body, options = {}) => {
  ...
  let json = `${between(body, "MUSIC_PAGE_TYPE_PLAYLIST\\\\\\x22\\x7d\\x7d\\x22\\x7d'), data: '", "'")}`;
  for (let hex of `${json}`.matchAll(/\\x([a-f\d]{2})/g)) {
    json = json.replace(hex[0], String.fromCharCode(parseInt(hex[1], 16)));
  }
  FS.writeFileSync(json, './output.txt');
