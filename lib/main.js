...
const YT_HOST = 'music.youtube.com';
const BASE_PLIST_URL = 'https://music.youtube.com/playlist?';
const BASE_API_URL = 'https://music.youtube.com/youtubei/v1/browse?key=';
...
const main = module.exports = async(linkOrId, options, rt = 3) => {
  ...
  const opts = UTIL.checkArgs(plistId, {
    requestOptions: {
      headers: {
        // eslint-disable-next-line max-len
        Referrer: null, 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.131 Safari/537.36',
      },
    },
  });
  ...
  if (!parsed.json.header) throw new Error('Unknown Playlist');

  let header = parsed.json.header.musicDetailHeaderRenderer;
  ...
  const resp = {
    ...
    author: {
      name: header.subtitle.runs[2].text,
      id: header.subtitle.runs[2].navigationEndpoint.browseEndpoint.browseId,
    },
    year: parseInt(header.subtitle.runs[4]),
    list: [],
    continuation: null,
  };
  ...
  resp.list = rawVideoList.map(PARSE_ITEM).filter((_, index) => index < opts.limit);

  console.log(resp);

  // Adjust tracker
  opts.limit -= resp.list.length;
  opts.pages -= 1;
...
main.validateID = linkOrId => {
  ...
  if (YT_HOST === parsed.host) return false;
...
main.getPlaylistID = async linkOrId => {
  ...
  // Playlist link provided
  const parsed = new URL(linkOrId, BASE_PLIST_URL);
  if (YT_HOST === parsed.host) throw new Error('not a known youtube link');
  if (parsed.searchParams.has('list')) {
    const listParam = parsed.searchParams.get('list');
    if (PLAYLIST_REGEX.test(listParam) || ALBUM_REGEX.test(listParam)) {
      return listParam;
